...
class YumpuAPI {
    ...
    /**
     * API Command für das Holen der Dokumente
     */
    const API_CMD_DOCUMENTS_GET = "documents.json";

    /**
     * API Command für das Holen eines Dokuments
     */
    const API_CMD_DOCUMENT_GET = "document.json";
    ...
    public function document_add($source_file, $title, $description = null) {
        if (phpversion() >= 5.5) {
            $file = curl_file_create($source_file, 'application/pdf', basename($source_file));
        } else {
            $file = '@' . $source_file;
        }
        $data = array(
    ...
    public function documents_get($offset = 0, $limit = 0, $sort = 'desc')
    {
        try {

            $result = json_decode($this->remote_get(self::API_CMD_DOCUMENTS_GET . '?offset=' . $offset . '&limit=' . $limit . '&sort=' . $sort . '&return_fields=id,create_date,update_date,url,short_url,image_small,image_medium,image_big,language,title,description,tags,embed_code,settings'));
    ...
    public function document_get($id)
    {
        try {
            ...
            $result = json_decode($this->remote_get(self::API_CMD_DOCUMENT_GET . '?id=' . $id));
